...
(defn make-potion
  ...
  [ingredient-seq & {:keys [use-remaining removed-effects remove-remaining]
                     :or {use-remaining nil removed-effects nil remove-remaining nil}}]
  (let [remaining-effects (map set (vals (select-keys @remaining use-remaining))) ;; [set]
        ...
        potion (map #(apply clojure.set/union (map clojure.set/intersection %)) (map set (combinations effs 2)))
